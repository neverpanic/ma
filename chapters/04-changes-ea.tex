...
\chapter{Escape Analysis}
	...
	\section{Basics}
		...
		\subsection{Intraprocedural Analysis}
			...
			For pointer analysis as discussed in~\cref{sec:intro:prev}, summarizing a method's effect independent of aliasing
			...
			liveness periods overlap are not allocated on the stack, because that would require moving the stack pointer
			...
			\inputthesiscode{java}{lst:ea:basics:local:fs}{Example exposing the difference between flow-sensitive and
				...
				Simple Java example exposing the different between flow-sensitive and flow-insensitive alias analysis.
				Flow-sensitive analysis will correctly determine that \texttt{c} will point to the \texttt{I} object at the end
				of the given method. Hence, escape analysis will determine that \texttt{H} and \texttt{J} do not escape
				\emph{run}. Flow-insensitive analysis will list \texttt{H}, \texttt{I}, and \texttt{J} as possible pointees of
				\texttt{c}. As a consequence, all of the three allocations will be assigned a \emph{global} escape state.
		...
		\subsection{Interprocedural Analysis}
			...
			field reference nodes \emph{head} and \emph{next} are added below \emph{obj0} and \emph{ListElement},
