...
\chapter{Escape Analysis}
	...
	\section{Basics}
		...
		\subsection{Intraprocedural Analysis}
			...
			the calling context. Since their allocation site might be unknown for some objects (e.g., if they have been passed
			as argument), a special type of placeholder called \emph{phantom node} is used to represent these objects. For
			pointer analysis as discussed in~\cref{sec:intro:prev}, summarizing a method's effect independently of aliasing
			relationships in the calling context is impossible~\cite[p.~886]{choi:03:toplas}. For each allocation, assignment,
			field or array access, return statement, method invocation, and exception throw, the \gls{cg} is modified
			appropriately, ensuring possible alias relations are represented accurately.
			...
			incoming deferred but no outgoing edges are preserved without change. \Cref{subsec:ea:improve:bug} gives the
			...
			in each KESO domain to ensure the isolation of the protection realms) or thrown as exceptions. While processing
			...
			not all of these objects are converted into stack allocations. Allocations inside loops that create objects whose
			...
			\begin{figure}
				...
				\subcaptionbox{%
					...
					passed to the \emph{insert} method at its invocation. \emph{ListElement(this)} and \emph{ListElement(elem)} do
					the same for the implicit invocation of \emph{ListElement}'s constructor.%
					\label{subfig:ea:basics:ll:addElement}}[.6\textwidth]{%
						\centering%
				...
				\subcaptionbox{%
					...
					into the caller context to determine this.\label{subfig:ea:basics:ll:insert}}[.38\textwidth]{%
						\centering%
			...
			\inputthesiscode{java}{lst:ea:basics:local:fs}{Example exposing the difference between flow-sensitive and
				flow-insensitive analysis.}{%
				Simple Java example exposing the difference between flow-sensitive and flow-insensitive alias analysis.
				Flow-sensitive analysis will correctly determine that \emph{c} will point to the \emph{I} object at the end of
				the given method. Hence, escape analysis will determine that \emph{H} and \emph{J} do not escape \emph{run}.
				Flow-insensitive analysis will list \emph{H}, \emph{I}, and \emph{J} as possible pointees of \emph{c}. As
				a consequence, all of the three allocations will be assigned a \emph{global} escape state.

				\flushright{}Adapted from~\cite[Sec.~12.4.3]{aho:07:compilers}.
			}{examples/FlowSensitivity.java}
			...
			the control flow and computes the possible pointees of each variable after each statement. In the given example,
		...
		\subsection{Interprocedural Analysis}
				...
				\Begin{%
					...
					\While{$\workList \ne \emptyset \;\mathrm{or}\; \needsPointee \ne \emptyset$}{%
						\While{$\workList \ne \emptyset$}{%
							...
							\BlankLine
							\tcp{Mark \iArg globally escaping if \mParam is.}
							$\updateEscapeState{\iArg, \mParam}$\;
							\BlankLine
			...
			\begin{figure}
				\centering%
				\input{examples/LinkedList-addElement-inter.tex}%
				\caption[The \glsentryshort{cg} for \emph{addElement} after interprocedural analysis]{%
	...
	\section{Improvements}
		...
		\subsection{Flow-Sensitivity}
			...
			After implementing this improvement, a fixed point iteration used to reduce the number of unnecessary phantom
		...
		\subsection{Fixing Incorrect Results: The Double Return Bug}
			...
			\inputthesiscode{java}{lst:ea:improve:bug}{Example exposing the double return flaw}{%
				A simplified example exposing the double return bug in the escape analysis by Choi et al. One of the objects
				allocated in \emph{getObject} is returned from its allocating method, but escape analysis did not detect this
				due to the use of phantom nodes to represent return values.
		...
		\subsection{Interprocedural Analysis Optimizations}
			...
			\subsubsection{Connection Graph Compression}
				...
				deterioration of the computed results up to this point. Each list that contains at least two object nodes, at
				least one of which must be a phantom node, is compressed by removing all phantom nodes. Note that any two
				non-phantom object nodes (i.e., any two nodes with a known allocation site) are not consolidated to preserve the
				one-to-one mapping between intermediate code allocation instruction and its \gls{cg} representation.
	...
	\section{Applications}
		...
		\subsection{Removing Unneeded Copies in Portal Calls}
			...
			portals, the KESO runtime environment creates deep copies of these objects in the target domain's heap. This
			ensures that modifications of the object in the target domain do not affect the object in the source domain.
			Especially for large trees of objects, this is very expensive. Results from escape and alias analysis can be used
			to determine whether the copy can be omitted. This optimization pass is called \emph{superfluous portal copy
			removal}.
			...
			Avoiding the removal of copies that are required because they are modified according to
		...
		\subsection{Synchronization Optimizations}
			...
			Since KESO does not currently synchronize at objects but uses OSEK's \emph{resource} abstraction for mutual
			...
			write operation will continue and bring the multi-word data field into a consistent state.
		...
		\subsection{Cycle-Aware Reference Counting}
			...
			Using the \acrlongpl{cg} constructed in alias and escape analysis, cyclic structures, their components and
			...
			\Cref{fig:ea:apps:cyclerefcnt}~has a graphical representation of the approach. The structure consisting of the
			...
			\begin{figure}
				\centering%
				\input{examples/cycledescriptors.tex}%
